...
class ServerCommand(server: OrangeMServer) extends Thread {
    ...
    def parseCommand(command: Array[String]): Unit = {
        commands.getOrElse(command(0), (_: Array[String]) => println(s"Command [${command(0)}] not found, use help " ))(command)
    }
